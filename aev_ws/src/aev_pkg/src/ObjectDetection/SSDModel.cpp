...
void SSDModel::threadDetectionStart()
{
    detection_thread = std::thread(&SSDModel::threadObjectDetection, this);
}
...
// Return the number of classes
bool SSDModel::isNewResult(void)
{
...
void SSDModel::threadObjectDetection()
{
    //int count = 0;
    ...
    while(true)
    {
		...
		if (detect_queue->isNewItem())
		{
			...
			current_image = detect_queue->receive();

			// Input the image to the model and get the result
			indices = detect(current_image, classIds, confidences, boxes);
