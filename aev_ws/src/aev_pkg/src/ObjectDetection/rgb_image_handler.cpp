...
void rgb_image_handler::obj_not_in_safe_area_hdl(void)
{
	if (count_obj_not_in_safe_area < 3)
	{
		count_obj_not_in_safe_area++;
	}
...
void rgb_image_handler::Camera_RGBframe_Handler(rs2::frameset _frameset)
{
	...
	{
		count_obj_not_in_safe_area = 0;
		obj_in_safe_area_hdl();
	}
	else
	{
		count_obj_in_safe_area = 0;
		obj_not_in_safe_area_hdl();
	...
	cv::line(image1, po1, po2, po_color, 2);
	cv::line(image1, po2, po3, po_color, 2);
	cv::line(image1, po3, po4, po_color, 2);
	imshow(window_detection_name, image1);
}
