...
#define CFG_DETECTION_Y_THRES  CFG_DETECTION_HEIGHT*10/20
#define CFG_DETECTION_X1_THRES  550
#define CFG_DETECTION_X2_THRES  (CFG_DETECTION_WIDTH - 500)
...
public:
	...
	void Camera_RGBframe_Handler(rs2::frameset _frameset);
...
private:
	...
	cv::Point po1 = cv::Point(300, CFG_DETECTION_HEIGHT);
    cv::Point po2 = cv::Point(CFG_DETECTION_X1_THRES, CFG_DETECTION_Y_THRES);
	cv::Point po3 = cv::Point(CFG_DETECTION_X2_THRES, CFG_DETECTION_Y_THRES);
    cv::Point po4 = cv::Point(CFG_DETECTION_WIDTH - 200, CFG_DETECTION_HEIGHT);
    cv::Scalar po_color = cv::Scalar(0, 0, 255);


	uint32_t count_detection_frame = 0;
	bool is_obj_in_safe_area = false;
	uint32_t count_obj_in_safe_area = 0;
	uint32_t count_obj_not_in_safe_area = 0;
	void obj_in_safe_area_hdl(void);
	void obj_not_in_safe_area_hdl(void);
