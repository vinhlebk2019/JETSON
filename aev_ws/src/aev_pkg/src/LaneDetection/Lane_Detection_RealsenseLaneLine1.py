...
center_line_width_div_factor = 3.5
...
def main():
    ...
    convertScale_alpha = 0.8 # Contrast control (1.0-3.0)
    convertScale_beta = -20 # Brightness control (-100 ->100)
    ...
    while True:
        ...
        if ret:
            ...
            org_frame = frame
            #cv2.imshow("Org Image", cv2.resize(org_frame, (image_out_width, image_out_height)))
            ...
            if (using_houghLine_flag == 1):
                ...
                if (x_c1 != invalid_value) and (x_c2 != invalid_value):
                    ...
                    if (x_c1 < 0) or (x_c1 > width) or (x_c2 < 0) or (x_c2 > width):
                        ...
                        x_c2 = width//center_line_width_div_factor
            ...
            if (x_l1 != invalid_value) and (x_l2 != invalid_value):
                ...
                if (x_l1 < width//2) or (x_l1 > width) or (x_l2 < 0) or (x_l2 > width):
                    x_l1 = width
                    x_l2 = width//left_line_width_div_factor
            ...
            if (abs(x_c2 - x_c1) > line_horizon_thres):
                ...
                x_center1 = x_c1
                x_center2 = x_c2
            #x_center2 = x_center2 + 50
            # x_center1 = x_center1 - 20

            # x_center1 = int(moving_average(1, x_center1))
            # x_center2 = int(moving_average(2, x_center2))
            ...
            cv2.line(frame, (int(x_left1 + x_center1) // 2, height), (int(x_left2 + x_center2) // 2, height - 200), (0, 0, 255), 10, cv2.LINE_AA)

            ####################################################################################################
            x_offset_center_and_left = 40
            if (output_data.houghline_available_center == 1) and (output_data.houghline_available_left == 1):
                finalCenterOffset = (round((x_c1 + x_l1)//2) - (width // 2)) * (width_to_meter / width)
            elif (output_data.houghline_available_center == 1):
                finalCenterOffset = (round(x_c1 + x_offset_center_and_left) - (width // 2)) * (width_to_meter / width)
            elif (output_data.houghline_available_left == 1):
                finalCenterOffset = (round(x_l1 - x_offset_center_and_left) - (width // 2)) * (width_to_meter / width)
            else:
                pass
            ...
            if (x_center2 - x_center1) == 0:
                ...
                radius = -1 / (x_center2 - x_center1) * 1500
            ...
            lane_detection_data.centerOffset = -finalCenterOffset * 0.7
