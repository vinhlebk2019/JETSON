...
# for bietthu
# yellow_HSV_th_min = np.array([0, 25, 0])
# yellow_HSV_th_max = np.array([20, 153, 160])
...
if __name__ == '__main__':
    cap = cv2.VideoCapture("video/VAS/vid_1.mp4")
    i = 0
    while cap.isOpened():
        ret, frame = cap.read()
        if ret:
            if i % 10 == 0:
                out_frame = binarize(img=frame, verbose=True)*255
                canny = cv2.Canny(out_frame, 180, 255)
