...
def extract_colour(img):
    ...
    sensitivity_hmax = 30 #40 #51 # cang nho cang den bau troi
...
def main(video_filename):
    ...
    while (cap.isOpened()):
        ...
        if ret:
            ...
            a, binary_img = cv2.threshold(binary_img, 127, 255, cv2.THRESH_BINARY)
            ...
            mask1 = get_mask(binary_img, width // 5, height // 2, 1, width // 2, height)
            ...
            if (using_polyfit_flag == 1):
                ...
                kernel = np.ones((20,1), np.uint8)
                cropped_image = cv2.dilate(cropped_image, kernel, iterations=1)
                cv2.imshow("polyfit Image 1", cv2.resize(cropped_image, (image_out_width, image_out_height)))
                frame1, x_low, x_high = polyfit_lane(cropped_image, frame, height, width)
                frame1[:,:,1] = frame1[:,:,1] * mask1
                cv2.imshow("frame1", cv2.resize(frame1, (image_out_width, image_out_height)))
                ...
                kernel = np.ones((20,1), np.uint8)
                cropped_image = cv2.dilate(cropped_image, kernel, iterations=1)
                #cv2.imshow("polyfit Image 2", cv2.resize(cropped_image, (image_out_width, image_out_height)))
                frame2, x_low, x_high = polyfit_lane(cropped_image, frame, height, width)
                #frame2[:,:,1] = frame2[:,:,1] * mask2
                #cv2.imshow("frame2", cv2.resize(frame2, (image_out_width, image_out_height)))
