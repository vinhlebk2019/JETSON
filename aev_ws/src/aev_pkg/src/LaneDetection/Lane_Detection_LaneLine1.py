...
class out_paras:
    def __init__(self):
        ...
        self.houghline_available_center = []
        self.houghline_available_left = []
...
# polyfit order
polyfit_order = 1
# center_line_width_div_factor
center_line_width_div_factor = 3.1
# left_line_width_div_factor
left_line_width_div_factor = 1.1
...
line_horizon_thres = 400
#x_offset_center_and_left_1
x_offset_center_and_left_1 = 300
x_offset_center_and_left_2 = 150
# Config algorithm 
using_houghLine_flag = 1 # 0 or 1
using_polyfit_flag = 0 # 0 or 1
using_Model_flag = 0 # 0 or 1
# Config ROI (mask) 
cfg_mask_width_factor = 4
cfg_mask_height_factor = 2.7
cfg_mask_height_derating_factor = 0.3
cfg_mask_center_shift_factor = 0
cfg_mask_devidied = 5
cfg_mask_offset_factor = 1.5
...
def extract_colour(img):
    ...
    sensitivity_vmin = 165 #140 #155 # cang to cang den con duong
...
def polyfit_lane(img_input_bw, img_rgb , height, width, mid_h, mid_w):
    ...
    line_seg_x_low = width//left_line_width_div_factor
    line_seg_x_high = width//left_line_width_div_factor
    ...
    #cv2.imshow("img_input_bw Image", cv2.resize(img_input_bw, (image_out_width, image_out_height)))
    try:
...
arr_moving_average_0 = [10000, 10000, 10000, 10000, 10000, 10000]
arr_moving_average_1 = [0, 0, 0, 0, 0, 0]
arr_moving_average_2 = [0, 0, 0, 0, 0, 0]
def moving_average(select, new):
    global arr_moving_average_0
    global arr_moving_average_1
    global arr_moving_average_2

    if (select == 0):
        arr_moving_average = arr_moving_average_0
    elif (select == 1):
        arr_moving_average = arr_moving_average_1
    elif (select == 2):
        arr_moving_average = arr_moving_average_2
    ...
    if (select == 0):
        arr_moving_average_0 = arr_moving_average
    elif (select == 1):
        arr_moving_average_1 = arr_moving_average
    elif (select == 2):
        arr_moving_average_2 = arr_moving_average
...
def main(video_filename):
    ...
    x_center1 = width//center_line_width_div_factor
    x_center2 = width//center_line_width_div_factor

    x_left1 = width
    x_left2 = width//left_line_width_div_factor
    ...
    while (cap.isOpened()):
        ...
        if ret:
            ...
            mask_1 = get_mask(ref_frame=binary_img, 
                                ...
                                x_point=x_center1 - cfg_mask_center_shift_factor*x_center_diff, 
                                y_point=height, 
            ...
            if (x_center_diff > 120):
                x_center_diff = 120
            elif (x_center_diff < -120):
                x_center_diff = -120

            if (using_houghLine_flag == 1):

                mask_center = get_mask(ref_frame=binary_img, 
                                    ...
                                    y_point=height, 
                                    offset=cfg_mask_offset_factor*x_center_diff)
                cropped_image = binary_img * mask_center
                ...
                if (x_c1 != invalid_value) and (x_c2 != invalid_value):
                    ...
                    if (x_c1 < 0) or (x_c1 > width) or (x_c2 < 0) or (x_c2 > width):
                        x_c1 = width//center_line_width_div_factor
                        x_c2 = width//center_line_width_div_factor + 100

                    if (abs(x_c1 - x_center1) > 200) or (abs(x_c2 - x_center2) > 200):
                        x_c1 = x_center1
                        x_c2 = x_center2
                    ...
                    output_data.houghline_available_center = 1
                else :
                    #print ("Failed")
                    #print (Random(1))
                    x_c1 = width//center_line_width_div_factor
                    ...
                    output_data.houghline_available_center = 0

                frame[:,:,1] = frame[:,:,1] * mask_center
                #cv2.imshow("Image Center", cv2.resize(frame, (image_out_width, image_out_height)))
            ...
            ##############################################################################################################
            x_left_diff = x_left2 - x_left1
            if (x_left_diff > 100):
                x_left_diff = 100
            elif (x_left_diff < -100):
                x_left_diff = -100
            
            mask_left = get_mask(ref_frame=binary_img, 
                                mask_width=(width // cfg_mask_width_factor // 1.5), 
                                mask_height=((height // cfg_mask_height_factor) - cfg_mask_height_derating_factor*abs(x_left_diff)), 
                                devidied=cfg_mask_devidied, 
                                x_point=(x_left2 + x_left1)/2 - cfg_mask_center_shift_factor*x_left_diff, 
                                y_point=height, 
                                offset=1.2*x_left_diff)
            cropped_image = binary_img * mask_left
            ...
            aperture_size = 5 # Aperture size, Aperture size of the Sobel filter.
            L2Gradient = True # Boolean, Boolean parameter used for more precision in calculating Edge Gradient.
            cropped_image = cv2.Canny(cropped_image, t_lower, t_upper, apertureSize = aperture_size, L2gradient = L2Gradient )

            frame, x_l1, x_l2 = hough_lines(cropped_image, frame)

            if (x_l1 != invalid_value) and (x_l2 != invalid_value):

                if (x_l1 < width//2) or (x_l1 > width) or (x_l2 < 0) or (x_l2 > width):
                    x_l1 = width//left_line_width_div_factor
                    x_l2 = width//left_line_width_div_factor - 100

                if (abs(x_l1 - x_left1) > 150) or (abs(x_l2 - x_left2) > 150):
                    x_l1 = x_left1
                    x_l2 = x_left2

                frame = cv2.circle(
                    img = frame, 
                    center = (round(x_l1), height), #width, height
                    radius = 5, 
                    color = (0,0,255),
                    thickness = 3)
                
                # output_data.offset_houghline = (round(x_l1) - (width // 2)) * (width_to_meter / width)
                output_data.houghline_available_left = 1
            else :
                x_l1 = width//left_line_width_div_factor
                x_l2 = width//left_line_width_div_factor - 100
                # output_data.offset_houghline = 0
                output_data.houghline_available_left = 0
            
            # frame11 = frame
            # frame11[:,:,1] = frame11[:,:,1] * mask_left
            # cv2.imshow("Image Left", cv2.resize(frame11, (image_out_width, image_out_height)))
            ...
            if (using_polyfit_flag == 1):
                ...
                '''
                mask_polyfit = get_mask(ref_frame=binary_img, 
                                mask_width=(width // cfg_mask_width_factor // 1.5), 
                                mask_height=((height // cfg_mask_height_factor) - cfg_mask_height_derating_factor*abs(x_left_diff)), 
                                devidied=cfg_mask_devidied, 
                                x_point=x_left1 - cfg_mask_center_shift_factor*x_left_diff, 
                                y_point=height, 
                                offset=cfg_mask_offset_factor*x_left_diff)

                cropped_image1 = binary_img * mask_polyfit
                ...
                aperture_size = 5 # Aperture size, Aperture size of the Sobel filter.
                L2Gradient = True # Boolean, Boolean parameter used for more precision in calculating Edge Gradient.
                cropped_image1 = cv2.Canny(cropped_image1, t_lower, t_upper, apertureSize = aperture_size, L2gradient = L2Gradient )

                frame, x_low, x_high = polyfit_lane(cropped_image1, frame, height, width, height//3, int(x_l2))
            ...
            #     pass
            # elif (x_c2 - x_c1) > 100: #direction_change_center_diff_thres:
            #     turn_direction = turn_right
            # elif (x_c2 - x_c1) < -50: #-direction_change_center_diff_thres:
            ...
            # if (x_center1 < 200) or (x_center1 > (width - 200)) or (x_center1 > 800) or (x_center2 > 800):
            #     x_center1 = width // center_line_width_div_factor
            #     x_center2 = width // center_line_width_div_factor

            # if (direction_history == turn_left):
            #     if (x_center2 > x_center1):
            #         x_center2 = x_center1 - 0
            # elif (direction_history == turn_right):
            #     if (x_center2 < x_center1):
            #         x_center2 = x_center1 + 50
            # else:
            #     x_center2 = x_center2# + 30
            ...
            if (x_center1 < 300) or (x_center1 > (width - 200)):
                x_center1 = width // center_line_width_div_factor
                x_center2 = width // center_line_width_div_factor

            #-----------------------------------------------------

            if (abs(x_l2 - x_l1) > line_horizon_thres):
                pass
            else:
                x_left1 = int(x_l1)
                x_left2 = int(x_l2)
            ...
            if (output_data.houghline_available_center == 1) and (output_data.houghline_available_left == 1):
                lane_point_1 = int(x_left1 + x_center1) // 2
                lane_point_2 = int(x_left2 + x_center2) // 2
            elif (output_data.houghline_available_center == 1):
                lane_point_1 = int(x_offset_center_and_left_1 + x_center1)
                lane_point_2 = lane_point_1
            elif (output_data.houghline_available_left == 1):
                lane_point_1 = int(x_left1 - x_offset_center_and_left_1)
                lane_point_2 = lane_point_1
            else:
                pass

            cv2.line(frame, (lane_point_1, height), (lane_point_2, height - 200), (0, 0, 255), 10, cv2.LINE_AA)

            if (abs(lane_point_2 - lane_point_1) > line_horizon_thres):
                pass
            elif (lane_point_2 - lane_point_1) > 80: #direction_change_center_diff_thres:
                turn_direction = turn_right
            elif (lane_point_2 - lane_point_1) < -80: #-direction_change_center_diff_thres:
                turn_direction = turn_left
            else:
            ...
            if (direction_history == turn_right):
                if (x_center2 < x_center1):
                    x_center2 = out_lane_data.last_center_1
            
            if (direction_history == turn_left):
                if (x_center2 > x_center1):
                    x_center2 = out_lane_data.last_center_2

            out_lane_data.last_center_1 = x_center1
            out_lane_data.last_center_2 = x_center2

            ####################################################################################################
            car_center = (width // 2) + 100
            if (output_data.houghline_available_center == 1) and (output_data.houghline_available_left == 1):
                finalCenterOffset = (round((x_c1 + x_l1)//2) - car_center) * (width_to_meter / width)
            elif (output_data.houghline_available_center == 1):
                finalCenterOffset = (round(x_c1 + x_offset_center_and_left_1) - car_center) * (width_to_meter / width)
            elif (output_data.houghline_available_left == 1):
                finalCenterOffset = (round(x_l1 - x_offset_center_and_left_1) - car_center) * (width_to_meter / width)
            else:
                finalCenterOffset = 0
                pass
            
            if (x_center2 - 100 - x_center1) == 0:
                radius = 500
            else:
                radius = -1 / (x_center2 -100 - x_center1) * 1500
            ...
            radius = abs(radius)
            if (radius >= 0) and (radius < 4):
                radius = 4
            else:
                radius = 4 + ((radius - 4) * 1.5)

            radius = radius_sign * moving_average(0, radius)
            ...
            output_img = cv2.arrowedLine(
                        img = output_img, 
                        pt1 = (car_center, height), 
                        pt2 = (car_center, height - 20),
                        color = (0,0,255), 
                        thickness = 2,
                        tipLength = 0.3)
            ...
            #out_msg = "{:.4f}".format(stop_frame_time - start_frame_time)
            #print (out_msg)
            #print(FPS)
...
if __name__ == '__main__':
    ...
    #video_name = "/home/ubuntu/aev/videos/20220428/3.mp4"

    #video_name = "/home/ubuntu/aev/videos/20220507/2.mp4"

    #video_name = "/home/ubuntu/aev/videos/20220603/3.mp4"

    video_name = "/home/ubuntu/aev/videos/20220611/1.mp4"
